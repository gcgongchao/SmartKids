package com.cmbb.smartkids.base.ui.card;
...
public class CardView extends FrameLayout {
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        mUpAdapterPostion = mListAdapter.getCount() - 1;
    ...
    private void ensureTopFull() {
        while (mUpAdapterPostion < mListAdapter.getCount()
                && getChildCount() < mMaxVisible) {
            int index = mUpAdapterPostion % mMaxVisible;
            View convertView = viewHolder.get(index);
            final View view = mListAdapter.getView(mUpAdapterPostion,
                    convertView, this);
            view.setOnClickListener(null);
            viewHolder.put(index, view);

            // 添加剩余的View时，始终处在前面
            index = Math.min(mUpAdapterPostion, mMaxVisible - 1);
//            ViewHelper.setScaleX(view, ((mMaxVisible - index - 1) / (float) mMaxVisible) * 0.2f +
//                    0.8f);
            int topMargin = (mMaxVisible - index - 1) * itemSpace;
            ViewHelper.setTranslationY(view, topMargin);
            ViewHelper.setAlpha(view, mUpAdapterPostion == 0 ? 1 : 0.5f);
...
//    }
    ...
    private boolean goDown() {
        ...
        ViewPropertyAnimator anim = ViewPropertyAnimator
                .animate(topView)
                .translationY(ViewHelper.getTranslationY(topView) + topView.getHeight()).alpha(0)
                .scaleX(1)
                .setListener(null).setDuration(2000);
    ...
    private boolean goUp() {
        //final View topView = getChildAt(getChildCount() - 1);
        final View lastView = getChildAt(0);
        if (!lastView.isEnabled()) {
            return false;
        }
        // topView.getHitRect(topRect); 在4.3以前有bug，用以下方法代替
        topRect = getHitRect(topRect, lastView);
        // 如果按下的位置不在顶部视图上，则不移动
        if (!topRect.contains((int) downX, (int) downY)) {
            return false;
        }
        ...
        ViewPropertyAnimator anim = ViewPropertyAnimator
                .animate(lastView)
                        //.translationY(ViewHelper.getTranslationY(lastView)+ lastView.getHeight()).alpha(0).scaleX(1)
                .translationY(0).alpha(0).scaleX(0.5f)
                .setListener(null).setDuration(2000);
        anim.setListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
     ...
     * @param view
     */
    private void bringToBehind(final View view) {
        topPosition++;
        float scaleX = ViewHelper.getScaleX(view) - ((float) 1 / mMaxVisible)
                * 0.2f;
        float tranlateY = ViewHelper.getTranslationY(view) + itemSpace;
